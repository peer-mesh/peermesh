...
test('builds idempotent on-demand wake and start jobs per requester bucket', () => {
  ...
  assert.equal(rows[0].idempotency_key, `provider-1:pm_abc:requester-1:on-demand:${getOnDemandWakeBucket(now)}:wake`)
  assert.equal(rows[1].idempotency_key, `provider-1:pm_abc:requester-1:on-demand:${getOnDemandWakeBucket(now)}:start`)
