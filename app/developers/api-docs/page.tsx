...
404: { "error": "Session not found" }`,
  ...
  --data '{ "sessionId": "sess_abc123" }'`,
    node: `await fetch('${V1}/session/end', {
  method: 'POST',
  headers: {
    Authorization: \`Bearer \${apiKey}\`,
    'Content-Type': 'application/json',
  },
  body: JSON.stringify({ sessionId: 'sess_abc123' }),
})`,
  ...
  --header 'Authorization: Bearer <token>'`,
    node: `const res = await fetch('${V1}/keys', {
...
for key in data['keys']:
  ...
  },
  {
    ...
    body: [
      ...
      { name: 'tier', type: 'string', required: false, desc: '"standard" | "advanced" | "enterprise" | "contributor". Default: "standard". Advanced+ require phone verification.' },
...
save_to_secure_storage(data['key'])`,
  },
  {
    ...
    body: [
      { name: 'id', type: 'string', required: true, desc: 'The key ID (e.g., "key_xyz") from /keys list.' },
...
403: { "quote": { "ok": false, "constraints": [{ "code": "tier_rpm_cap", "message": "Standard tier is capped at 120 RPM" }] } }`,
  ...
  --data '{
    ...
    node: `const res = await fetch('${V1}/billing/quote', {
  method: 'POST',
  headers: {
    Authorization: \`Bearer \${token}\`,
    'Content-Type': 'application/json',
...
const GROUPS = [
  { label: 'Core', ids: ['version'] },
  { label: 'Sessions', ids: ['session-create', 'session-end'] },
        ...
        <div style={{ maxWidth: '900px', width: '100%' }}>
          ...
          <section>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
              ...
              <span style={{ fontSize: '13px', color: 'var(--text)', fontWeight: 600 }}>200 OK</span>
            </div>
